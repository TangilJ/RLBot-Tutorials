...
class agent:
    def __init__(self, team):
        ...
        # The minimum distance the ball needs to be away from the bot for the bot to boost
    ...
    def aim(self, target_x, target_y):
        ...
        if abs(angle_front_to_target) > self.POWERSLIDE_ANGLE:
            self.powerslide = 1
        else:
            self.powerslide = 0
    ...
    def get_output_vector(self, values):
        ...
        if (self.team == "blue" and self.bot_pos_y < self.ball_pos_y) or (self.team == "orange" and self.bot_pos_y > self.ball_pos_y):
            ...
            if self.distance(self.bot_pos_x, self.bot_pos_y, self.ball_pos_x,
                             self.ball_pos_y) > self.DISTANCE_FROM_BALL_TO_BOOST:
                self.boost = 1
            else:
                self.boost = 0
        ...
        if self.ball_pos_x == 0 and self.ball_pos_y == 0:
            self.aim(self.ball_pos_x, self.ball_pos_y)
            self.boost = 1
            self.acceleration = 32767
